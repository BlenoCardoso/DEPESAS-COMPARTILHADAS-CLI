...
export default function MobileLayout({ children }: MobileLayoutProps) {
  ...
  return (
    <div className="min-h-screen bg-muted/40">
      <div className="mx-auto flex min-h-screen w-full max-w-5xl flex-col bg-background shadow-none lg:rounded-3xl lg:border lg:border-border/50 lg:shadow-2xl">
        <header className="sticky top-0 z-50 w-full border-b bg-card/85 backdrop-blur-xl safe-area-top">
          <div className="mx-auto flex h-14 w-full max-w-5xl items-center justify-between px-4 sm:h-16">
                ...
                <div className="flex h-full flex-col">
                  ...
                  <div className="space-y-2 border-t p-4">
                    <Link href="/settings">
                      <div
                        onClick={() => setIsMenuOpen(false)}
                        className="flex items-center gap-3 rounded-lg px-4 py-3 transition-all cursor-pointer hover:bg-muted"
                      >
            ...
            <Link href="/">
              <div className="flex cursor-pointer items-center gap-2">
                ...
                <span className="hidden text-base font-bold sm:inline sm:text-lg">{APP_TITLE}</span>
              </div>
            </Link>
        ...
        <main className="flex-1 w-full max-w-5xl mx-auto px-4 py-5 safe-area-bottom">
          <div key={location} className="page-transition">
            {children}
          </div>
        </main>
        <nav
          className={`${forceMobile ? "" : "md:hidden"} sticky bottom-0 z-40 w-full border-t bg-card/90 backdrop-blur-xl safe-area-bottom`}
        >
          <div className="mx-auto flex h-14 w-full max-w-5xl items-center justify-around px-2 sm:px-4">
            {[
              { icon: Home, path: "/", label: "Home" },
              { icon: Users, path: "/groups", label: "Grupos" },
              { icon: CreditCard, path: "/shared-expenses", label: "Despesas" },
              { icon: PieChart, path: "/reports", label: "Relatórios" },
            ].map((item) => {
              const Icon = item.icon;
              const isActive = location === item.path;
              return (
                <Link key={item.path} href={item.path}>
                  <div
                    className={`flex flex-col items-center justify-center gap-1 rounded-2xl px-3 py-1.5 text-xs transition-all duration-200 ease-out cursor-pointer ${
                      isActive
