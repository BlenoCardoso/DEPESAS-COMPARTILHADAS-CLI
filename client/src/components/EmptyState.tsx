...
const DefaultIcon = () => (
  ...
  >
    <rect x="12" y="18" width="96" height="72" rx="18" fill="currentColor" opacity="0.18" />
    <path
...
export function EmptyState({ title, description, icon, cta, hint, className }: EmptyStateProps) {
  return (
    <div className={cn("empty-state glass-panel", className)}>
      ...
      >
        <div className="rounded-2xl bg-primary/10 p-4 text-primary/70">
          {icon ?? <DefaultIcon />}
        </div>
        <div className="space-y-2">
          <h3 className="text-lg font-semibold tracking-tight text-foreground">{title}</h3>
          <p className="text-base text-muted-foreground max-w-sm">{description}</p>
        </div>
